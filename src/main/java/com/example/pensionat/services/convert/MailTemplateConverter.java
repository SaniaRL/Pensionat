...
public class MailTemplateConverter {
    ...
    public static MailTemplate mailTemplateDTOtoMailTemplate(MailTemplateDTO m){
        return MailTemplate.builder().id(m.getId()).name(m.getName()).subject(m.getSubject()).body(m.getBody()).build();
    }

    public static MailTemplateDTO mailTemplateToMailTemplateDTO(MailTemplate m){
        return MailTemplateDTO.builder().id(m.getId()).name(m.getName()).subject(m.getSubject()).body(m.getBody()).build();
    }
}
